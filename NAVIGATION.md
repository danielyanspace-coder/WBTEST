...
- [x] **Этап 16 — Финансы**: ОПиУ (P&L) + юнит-экономика с налогами + маржа по
      каждому товару (`lib/finance/*`, `/dashboard/finance`, `finance_settings`).
- [x] **Этап 17 — Мониторинг приёмки складов**: пользователь сам выбирает склады
      и лимит коэффициента; Telegram-алерт «открылась приёмка»
      (`warehouse_watches`, Supplies API, cron-проверка, виджет в «Поставках»).
