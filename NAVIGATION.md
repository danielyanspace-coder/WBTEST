...
> Что осталось «дорастить» позже: биддер рекламы и SEO-генерация (UI готов),
> внешняя аналитика ниш (этап с парсингом — отдельно), cron-автосинхронизация,
> выплаты рефералам через ЮKassa Payouts (сейчас помечаются вручную).
...
lib/data/dashboard.ts # агрегаты для страниц кабинета
app/actions/*         # auth, store (WB), reviews, billing, referral
